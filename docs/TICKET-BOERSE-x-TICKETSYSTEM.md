...
**Und die Lifecycle-Falle (Runde 4b): ✅ ERLEDIGT (18.07.).** Der Fix liegt genau dort, wo er hingehörte — im Ticket-System: `entitlementsForOrder` schließt Testbestellungen (`test: true`) seit 18.07. **global** aus, `/api/verkaufszahl` ist gemergt und live. Testbestellungen erreichen die Ticket-Zahl gar nicht mehr; `ignoredTickets` ist im Ticket-Modus obsolet (bleibt nur für den Bestands-Notpfad relevant). Die ursprüngliche Falle: eine neutralisierte Testbestellung, die später storniert wurde, hätte den Kurs dauerhaft um die Testmenge zu niedrig gehalten.
